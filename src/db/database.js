        ...
        Address.findOne({user_id: transaction.user_id, wallet_type: transaction.wallet_type},async (er,addrFound)=>{
            if(addrFound){
                ...
                Address.updateOne({_id:addrFound._id}, {$set: updObj }, (err, result) =>{
                    ...
                    Ledger.updateOne({uniq_id:transaction.uniq_id,transaction_hash:transaction.transaction_hash, wallet_type:transaction.wallet_type},{$set:{status:1, confirmed: true, block_confirmations: confirmations}},(er, updated)=>{
                        if(er) {resolve(null)}
                        else{
...
const getPendingDeposits = function(){
    return new Promise((resolve, reject) =>{
        Ledger.find({status: 0, confirmed: false,activity_type: 'Deposit', wallet_type:{$in:['ETH', 'SML','USDT']}}).limit(50).exec(function(er, result){
            if(er){ resolve(null)}
            else{ resolve(result)}
        }) 
    })
...
const getOpenDeposits = function(){
    return new Promise((resolve, reject)=>{
        Ledger.distinct("to_address",{$or: [ { is_taken: { $exists: false } }, { is_taken: false } ], value:{$gt:100},activity_type: 'Deposit', wallet_type:{$in:['USDT']}}).exec(function(err, result){
            if(err)
                resolve([])
            else
                resolve(result)
...
module.exports.updatePhase = function(stats, phase){
    return new Promise((resolve, reject)=>{
        Transfers.findOneAndUpdate({_id: phase._id, confirmed: false, active_phase: true}, {$set:{confirmed: true, active_phase: false, status: true, ETH: stats.ETH,  USDT: stats.USDT}}).exec(function(err, result){
            if(err){
                resolve(null)
            }else{
                resolve(result)
...
// w.save()
// var l = new BlockTracker({coin_type:'ETH', block_number:7997362, short_name:'ETH'})
// l.save()
// var l1 = new BlockTracker({coin_type:'USDT', block_number:7997362, short_name:'USDT'})
// l1.save()
// l2.save()
